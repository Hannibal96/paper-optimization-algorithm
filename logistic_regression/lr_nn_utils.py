...
class F_nn(nn.Module):
    def __init__(self, d):
        super().__init__()
        self.linear_layer_1 = nn.Linear(in_features=d, out_features=d, bias=False)
        self.linear_layer_2 = nn.Linear(in_features=d, out_features=1, bias=False)
...
class Q_nn(nn.Module):
    def __init__(self, r):
        super().__init__()
        self.linear_layer_1 = nn.Linear(in_features=r, out_features=r, bias=False)
        self.linear_layer_2 = nn.Linear(in_features=r, out_features=1, bias=False)

    def forward(self, z: torch.Tensor):
        z = self.linear_layer_1(z)
        z = F.relu(z)
        z = self.linear_layer_2(z)
        return torch.sigmoid(z)
...
class R_nn(nn.Module):
    def __init__(self, d, r):
        super().__init__()
        self.linear_layer_1 = nn.Linear(in_features=d, out_features=d, bias=False)
        self.linear_layer_2 = nn.Linear(in_features=d, out_features=r, bias=False)
...
def find_q(r, x, r_nn, f_nn, lr, tol=1e-2, T=100):
    ...
    for t in range(T):
        ...
        q_loss.backward()
        optimizer.step()
        if torch.linalg.norm(q_nn.linear_layer_1.weight.grad) + \
                torch.linalg.norm(q_nn.linear_layer_1.weight.grad) < tol:
            break
    return q_nn
